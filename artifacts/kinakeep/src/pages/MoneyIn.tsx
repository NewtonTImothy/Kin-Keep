...
export default function MoneyIn() {
  ...
  const { data: categories = [] } = useListCategories(businessId, { type: 'income' }, { query: { enabled: !!businessId } });
  const { data: customers = [] } = useListCustomers(businessId, {}, { query: { enabled: !!businessId } });
    ...
    try {
      ...
      await createTxn.mutateAsync({
        data: {
          ...
          customerId: values.customerId && values.customerId !== 'none' ? parseInt(values.customerId, 10) : undefined,
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-display font-bold text-foreground">Money In</h1>
        <p className="text-muted-foreground">Record a new sale, payment, or income.</p>
      </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="What was this for?" className="h-12" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div className="pt-4 flex items-center justify-end gap-3">
                ...
                <Button type="submit" size="lg" className="min-w-[120px]" disabled={createTxn.isPending || isUploading}>
                  {createTxn.isPending || isUploading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Save Income'}
                </Button>
              </div>
